...
import { Camera, ChevronRight, ChevronLeft, Plus, Minus, Sparkles, AlertCircle } from "lucide-react";
...
export default function StepLens() {
  ...
  const handleNext = () => {
    ...
    localSelections.forEach((selection) => {
      const existing = state.selections.lens.find(
      ...
      if (!existing && selection.quantity > 0) {
        dispatch({
          type: "ADD_SELECTION",
          payload: { step: "lens", equipment: selection },
        });
      } else if (existing) {
        dispatch({
  ...
  // 선택된 카메라 정보 표시
  const selectedCameras = state.selections.camera
    ...
    <div className="p-6 space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-100 rounded-xl p-4">
        <p className="text-sm text-blue-900 leading-relaxed">
          ...
          {selectedCameras ? (
            <>
              <strong>{selectedCameras}</strong>에 호환되는 렌즈를 추천합니다.
            </>
          ) : (
            "선택한 카메라에 맞는 렌즈를 추천합니다."
          )}
        </p>
      </div>
      ...
      {isLoading && (
        <div className="flex flex-col items-center justify-center py-12 space-y-3">
          ...
          <p className="text-sm text-gray-500">AI가 최적의 렌즈를 분석중입니다...</p>
        </div>
      )}
      ...
      {error && (
        <div className="bg-red-50 border border-red-100 rounded-xl p-4 flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <button
              onClick={() => fetchRecommendation("lens")}
      ...
      {recommendation && recommendation.alternatives.length > 0 && (
        <div className="space-y-4">
          <h3 className="text-sm font-bold text-gray-700">대안 렌즈</h3>
