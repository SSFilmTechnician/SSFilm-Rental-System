...
export default function AssetManagement() {
  ...
  const latestVersion = useQuery(api.changeHistory.getLatestVersion);
        ...
        <div className="border-t pt-6 mt-6">
          <div className="flex items-center justify-between mb-4">
            <p className="text-sm text-gray-500">
              총 <span className="font-bold text-gray-900">{filteredEquipment.length}</span>개 항목
          ...
          {filteredEquipment.length > ITEMS_PER_PAGE && (
            ...
            {(() => {
              ...
              const canGoPrev = currentGroup > 1;
              return canGoPrev ? (
                <button
                  onClick={() => setCurrentPage((currentGroup - 2) * 5 + 1)}
            ...
            {(() => {
              ...
              const canGoNext = currentGroup < lastGroup;
              return canGoNext ? (
                <button
                  onClick={() => setCurrentPage(currentGroup * 5 + 1)}
                  className="px-2 py-1 text-gray-400 hover:text-gray-700 transition-colors"
                >
